...
app.post(
  ...
  async (req, res) => {
    ...
    const { title, description, keyFeatures, targetAudience, revenueModel } =
      req.body;
    const userId = req.user.userId;
    try {
      const result = await sql`
        INSERT INTO ideas (user_id, title, description, key_features, target_audience, revenue_model)
        VALUES (${userId}, ${title}, ${description}, ${JSON.stringify(
        keyFeatures
      )}, ${targetAudience}, ${revenueModel})
        RETURNING id
      `;
...
app.get("/api/ideas", limiter, authenticate, async (req, res) => {
  ...
  try {
    ...
    `;
    const totalCount =
      await sql`SELECT COUNT(*) FROM ideas WHERE user_id = ${userId}`;
    res.json({
      ideas: result.rows.map((idea) => ({
        ...idea,
...
app.put(
  ...
  async (req, res) => {
    ...
    try {
      const result = await sql`
        UPDATE ideas
        SET title = ${title}, description = ${description}, key_features = ${JSON.stringify(
        keyFeatures
      )},
      ...
      if (result.rows.length === 0) {
        return res
          .status(404)
          .json({ error: "Idea not found or unauthorized" });
      }
...
app.get("/api/documents", limiter, authenticate, async (req, res) => {
  ...
  try {
    ...
    `;
    const totalCount =
      await sql`SELECT COUNT(*) FROM documents WHERE user_id = ${userId}`;
    res.json({
...
app.put(
  ...
  async (req, res) => {
    ...
    try {
      const result = await sql`
        UPDATE documents
        SET type = ${type}, title = ${title}, content = ${JSON.stringify(
        content
      )}
        WHERE id = ${id} AND user_id = ${userId}
        RETURNING *
